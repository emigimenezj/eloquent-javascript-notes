...
you will end up defining a local one with the same name instead. Some languages work like this by design,
...
updating the binding in an outer scope if it doesn’t already exist in the inner scope.
If the binding is not defined at all, throw a ReferenceError (another standard error type).
...
specialForms.set = (args, scope) => {
  ...
  if (args.length != 2 || args[0].type != "word") {
    return new ReferenceError("nope1");
  }
  ...
  if (scope[name]) {
    ...
    throw new ReferenceError("nope2");
  }
};
/*
run(`
